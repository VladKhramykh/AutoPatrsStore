    ...
    <v-container>
        <v-row>
            <v-col cols="4" md="3">
                ...
                <search-form :parts="parts" :search="search" :searchedCount="searchedCount"/>
            </v-col>
            <v-col cols="12" md="8">
                <v-row>
        ...
        methods: {
            selectSubType: function (partSubtype) {
                ...
                this.$http.get(url)
                    .then(function (data) {
                        if (data.ok) {
                            this.searchedCount = data.body.length;
                            this.parts = data.body;
                        }
                    })
            },
            search: function (searchDto) {
                this.$http.post("http://localhost:8081/api/car-parts/search", searchDto)
                    .then(function (data) {
                        if (data.ok) {
                            this.searchedCount = data.body.length;
                            this.parts = data.body;
